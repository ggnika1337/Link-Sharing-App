...
function LinkButton({
  ...
  return (
    <a
      ...
      style={{ background: colors[link.platform] || "#333" }}
      href={link.url}
      target="_blank"
      rel="noreferrer"
    >
      <span>
        <b className="platform-symbol">{symbols[link.platform] || "↗"}</b>
        {link.platform}
      </span>
      <span>→</span>
    </a>
...
function ProfileVisual({
  ...
  return (
    <div className={preview ? "preview-card" : "phone"}>
      {user?.avatar?.url ? (
        <img className="avatar" src={user.avatar.url} alt="Profile" />
      ) : (
...
export default function DevLinksApp() {
  ...
  const [route, setRoute] = useState<Route>(() =>
    typeof window !== "undefined" && window.location.pathname === "/preview"
      ? "preview"
      : "editor",
  );
  ...
  useEffect(() => {
    ...
    const onPopState = () =>
      setRoute(window.location.pathname === "/preview" ? "preview" : "editor");
  ...
  async function loadAccount(activeToken = token) {
    ...
    try {
      const [account, savedLinks] = await Promise.all([
        request<User>("/auth/current-user", {}, activeToken),
        request<LinkItem[]>("/links", {}, activeToken),
      ]);
      setUser(account);
      setLinks(savedLinks);
    } catch (error) {
  ...
  function go(next: Route) {
    setNotice("");
    setRoute(next);
    window.history.pushState({}, "", next === "preview" ? "/preview" : "/");
  }
  function signOut() {
    localStorage.removeItem("devlinks-token");
    setToken(null);
  ...
  if (route === "preview")
    return <Preview user={user} links={links} onBack={() => go("editor")} />;
  if (!token || !user)
    return (
      <Auth
        mode={authMode}
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <button className="outline-btn" onClick={() => go("preview")}>
          Preview
        </button>
      </header>
      <section className="workspace">
        <aside className="phone-panel">
          <ProfileVisual user={user} links={links} />
        </aside>
        {route === "profile" ? (
          <ProfileEditor
      ...
      <div className="editor-body">
        ...
        {links.length === 0 ? (
          ...
          links.map((link, index) => (
            <div className="link-card" key={link._id || index}>
              ...
              <label className="field">
                ...
                <input
                  className={`field-input ${errors[index] ? "invalid" : ""}`}
...
function ProfileEditor({
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      let updated = await request<User>(
        `/users/${user._id}`,
      ...
      <div className="editor-body">
        <div className="image-row">
          ...
          <label className="upload-box">
            <span>{file || user.avatar?.url ? "✓" : "▧"}</span>
          ...
          <label className="field">
            <input
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  fullName: `${e.target.value} ${rest.join(" ")}`.trim(),
                })
              }
            />
          </label>
...
function Preview({
  ...
  return (
    <main className="preview-page">
      ...
      <ProfileVisual user={user} links={links} preview />
    </main>
  );
}
